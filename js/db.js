...
// ──────────────────────────────────────────────────────────────────
/**
 * Merge stored settings over defaults. Crucially this DEEP-merges `zone`,
 * so a partially-stored zone (e.g. one missing `radius`) can never leave
 * `radius` undefined — which previously made the employee gate compare
...
function mergeSettings(data) {
  ...
  return {
    ...DEFAULT_SETTINGS,
    ...d,
    zone: { ...DEFAULT_SETTINGS.zone, ...(d.zone || {}) },
  };
}
...
export async function saveSettings(partial) {
  ...
  // Deep-merge zone so a partial zone update never drops sibling fields.
  if (partial && partial.zone) {
    updated.zone = { ...current.zone, ...partial.zone };
  }
...
export function watchSettings(callback) {
  return onSnapshot(settingsRef(), (snap) => {
    callback(snap.exists() ? mergeSettings(snap.data()) : { ...DEFAULT_SETTINGS });
  });
}
