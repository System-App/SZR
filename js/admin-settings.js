...
function buildZoneSection(settings) {
  ...
  // Robustly force Leaflet to recompute its size. A map created inside a
  // container that was display:none (or still animating/laying out) reads a
  // wrong size and only tiles part of the view — the bug where the map only
  // fills a corner. We fix it two complementary ways:
  //   1. A few delayed invalidateSize passes (covers slow layout/animation).
  //   2. A ResizeObserver that fires whenever the container's box changes,
  //      so the moment the map becomes visible/full-size it re-tiles.
  // Together these guarantee the map fills its container on every device.
  const ensureMapSized = () => {
    if (!map) return;
    const kick = () => { if (map) map.invalidateSize(); };
    requestAnimationFrame(kick);
    setTimeout(kick, 200);
    setTimeout(kick, 500);
    setTimeout(kick, 1000);
    if (window.ResizeObserver && !mapEl._ro) {
      const ro = new ResizeObserver(() => kick());
      ro.observe(mapEl);
      mapEl._ro = ro; // keep a handle so we don't observe twice
    }
  };
  ...
  const initMap = async () => {
    ...
    // Fix map size once it becomes visible / full-size (timing-proof).
    ensureMapSized();
  };
  ...
  locBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const base = err instanceof GeoError ? err.userMessage : 'کێشە لە دۆزینەوەی شوێن';
      // On a laptop/desktop GPS is usually unavailable, so nudge the admin to
      // place the office pin by hand on the map instead.
      toast(base + ' یاخود لەسەر نەخشە شوێنی ئۆفیس بەدەستی دیاری بکە (کلیک لەسەر نەخشە).', 'error', 6500);
    } finally {
