...
export async function renderSettings(main) {
  ...
  wrap.appendChild(el('div', { style: { textAlign: 'center', padding: '12px', color: 'var(--ink-faint)', fontSize: 'var(--text-xs)' } }, [
    el('div', { style: { fontWeight: '700', color: 'var(--ink-soft)' } }, 'SZR — Shkoy Zawy Real Estate'),
    el('div', { style: { marginTop: '2px' } }, 'وەشانی 1.0.0'),
  ]));
...
function buildZoneSection(settings) {
  ...
  const updateMapMarker = () => {
    ...
    else circle = L.circle([lat, lng], { radius, color: '#2C3828', fillColor: '#8BAF6E', fillOpacity: 0.2, weight: 2 }).addTo(map);
  };
  ...
  locBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      renderCoords();
      if (map) { map.setView([lat, lng], 16); updateMapMarker(); }
      toast(`شوێن وەرگیرا (وردی: ${Math.round(pos.accuracy)}m). دەتوانیت لەسەر نەخشە بیجوڵێنیت.`, 'success', 4000);
      haptic(15);
    } catch (err) {
