...
function openEmployeeModal(existing = null) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      if (isEdit) {
        // Update profile (name + photo). Passwords are changed by the
        // employee themselves — the client SDK can't set another user's
        // password without the Admin SDK / a Cloud Function.
  ...
  const body = [
    ...
    isEdit
      ? el('div.info-note', {}, 'گۆڕینی وشەی نهێنی تەنها لەلایەن خودی کارمەندەوە دەکرێت لە بەشی «ڕێکخستنەکان»ی ئەو. ئەگەر کارمەند وشەکەی لەبیرکردووە، دەتوانیت بیسڕیتەوە و بە هەمان کۆد و وشەیەکی نوێ دروستی بکەیتەوە.')
      : el('div.field', {}, [
          el('span.field__label', {}, 'وشەی نهێنی'),
          pwInput,
        ]),
  ];
...
async function handleDelete(emp) {
  const ok = await confirmModal({
    ...
    message: `دڵنیایت لە سڕینەوەی ${emp.name}؟ هەموو تۆمارەکانی دەوامیشی دەسڕێنەوە. ئەم کردارە ناگەڕێتەوە. تێبینی: ئەگەر دواتر هەمان کۆد دووبارە بەکاربهێنیتەوە، پێویستە هەمان وشەی نهێنی پێشوو بەکاربهێنیت.`,
    confirmText: 'سڕینەوە',
    danger: true,
  });
  if (!ok) return;
