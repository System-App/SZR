...
async function handleCheckin(btn, container, profile, settings, today) {
  ...
  if (settings.zone?.enabled) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        gpsLine.innerHTML = '';
        // Show distance, plus a hint when the GPS fix itself is weak so the
        // employee knows to move into the open and retry rather than assume
        // the system is wrong.
        const weakGps = res.accuracy != null && res.accuracy > 40;
        const hint = weakGps
          ? ` (وردی GPS لاوازە: ${res.accuracy}m — لە شوێنێکی کراوەتر هەوڵ بدە)`
          : '. تکایە نزیک ببەرەوە.';
        gpsLine.append(
          el('span', { html: alertIcon() }),
          el('span', {}, `تۆ لە دەرەوەی ناوچەی دەوامیت (نزیکەی ${res.distance}m دوور)${hint}`)
        );
        resetButton(btn, 'in');
        haptic([30, 50, 30]);
        return;
  ...
  // Record check-in (server-authoritative time + GPS location)
  setButtonBusy(btn, 'تۆمارکردن...');
  try {
    const time = nowTime();
    await setAttendance(profile.id, today, { checkIn: time, checkInLoc, serverTime: true });
...
// ──────────────────────────────────────────────────────────────────
// CHECK-OUT HANDLER (GPS verified, same as check-in)
// ──────────────────────────────────────────────────────────────────
async function handleCheckout(btn, container, profile, settings, today) {
  haptic(15);
  setButtonBusy(btn, 'پشکنینی شوێن...');

  // GPS verification (if zone enabled) — same rule as check-in so an
  // employee can't clock out from outside the work area.
  ...
  if (settings.zone?.enabled) {
    ...
    try {
      ...
      checkOutLoc = { lat: res.lat, lng: res.lng, accuracy: res.accuracy, distance: res.distance };
    } catch (err) {
      gpsLine.className = 'gps-status gps-status--error';
      gpsLine.innerHTML = '';
      const msg = err instanceof GeoError ? err.userMessage : 'کێشەیەک ڕوویدا لە دۆزینەوەی شوێن';
  ...
  // Record check-out (server-authoritative time + GPS location)
  setButtonBusy(btn, 'تۆمارکردن...');
  try {
    const time = nowTime();
    await setAttendance(profile.id, today, { checkOut: time, autoCheckout: false, checkOutLoc, serverTime: true });
