...
export async function renderReports(main) {
  ...
  main.appendChild(el('div.date-range', {}, [
    ...
    el('label', { style: { display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '12px' } }, [
      el('span', { style: { fontSize: 'var(--text-xs)', color: 'var(--ink-soft)', fontWeight: '600' } }, 'بەپێی کارمەند'),
      empSelect,
    ]),
  ]));
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  th { background:#2C3828; color:#F5EFE3; font-weight:700; font-size:12px; }
  ...
  tr.total { background:#F8F6F1; font-weight:800; }
...
<body>
  ...
  <div class="footer">Shkoy Zawy Real Estate © 2026 — سیستەمی دەوام</div>
</body>
</html>`;

  // Print via a hidden iframe. window.open + print is frequently blocked on
  // iOS Safari (popup blocker), whereas an in-document iframe prints reliably
  // on both iOS and desktop without needing popup permission.
  ...
  const triggerPrint = () => {
    ...
    try {
      ...
      win.focus();
      // Remove the iframe once printing finishes (where supported).
      win.addEventListener?.('afterprint', cleanup);
      win.print();
      // Fallback cleanup in case afterprint never fires.
      setTimeout(cleanup, 60000);
    } catch (e) {
      console.warn('[SZR] Print error:', e);
      cleanup();
      toast('کێشە لە ئامادەکردنی پرینت', 'error');
    }
  };
  ...
  // Safari sometimes doesn't fire onload for document.write; fall back.
  setTimeout(() => { if (!cleaned && iframe.contentWindow) triggerPrint(); }, 1200);
...
function exportCSV(summaries, startDate, endDate) {
  ...
  // BOM for Excel UTF-8 + RTL friendliness. Each cell is quoted, and any
  // cell starting with = + - @ (or tab/CR) is prefixed with a single quote
  // so spreadsheet apps don't execute it as a formula (CSV injection guard).
  const safeCell = (cell) => {
    let s = String(cell);
    if (/^[=+\-@\t\r]/.test(s)) s = "'" + s;
    return `"${s.replace(/"/g, '""')}"`;
