...
async function setupSession(user) {
  ...
  // ONLY the admin runs this — the admin has permission to read all
  // attendance records. Employees can only read their own, so running it
  // for them would trigger a permissions error (and they don't need it).
  if (state.role === 'admin') {
    runAutoCheckout(state.settings).then((n) => {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('sw.js').catch((e) =>
        console.warn('[SZR] SW registration failed:', e)
      );
    });
  }
}
